...
def optimV2(sys, approx_theta=False, approx_hc=True):
    ...
    sys.V = sol[0]
    fsol = solver(sys, E, x, approx_theta=approx_theta,
                  approx_hc=approx_hc).heatCurrent()
...
def optimL2(sys, approx_theta=False, approx_hc=False):
    ...
    x = linspace(0, sys.L, 5)
    Ndef = 350
    solv = solver.solver(sys, approx_theta=approx_theta,
                         approx_hc=approx_hc, N=Ndef)
    ...
    sys.L = sol[0]
    fsol = solver.solver(sys, approx_theta=approx_theta,
                         approx_hc=approx_hc, N=Ndef).heatCurrent()
...
def minT(sys, approx_theta=False, approx_hc=False, optV=False, short_approx=False):
    def fun(tn):
        ...
        hc = solver.solver(syss, N=300, approx_theta=approx_theta,
                           approx_hc=approx_hc, short_approx=short_approx).heatCurrent()
...
def optimL(sys, approx_hc=False, approx_theta=False, short_approx=False, optV=False):
    def minfun(l):
        sys.L = abs(l)
        try:
            mint = minT(sys, approx_theta=approx_theta,
                        approx_hc=approx_hc, short_approx=short_approx, optV=optV)
        except ValueError:
            mint = .5

    sol = fmin(minfun, 1., xtol=.05, disp=False)
    sys.L = sol[0]
    fsol = minT(sys, approx_theta=approx_theta, approx_hc=approx_hc,
                short_approx=short_approx, optV=optV)
    return array((sol, fsol), dtype=float)
