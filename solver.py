...
class solver:
    def __init__(self, geometry, E=None, x=None,N=500, shortappr = False, longappr = False, approx_hc=False, approx_theta=False, short_approx=False,bulkappr=False):
        ...
        if self.E == None:
            ...
            de = 3./N
            self.E = concatenate((linspace(0.000001,0.95,round(N/3)),linspace(0.95+de,1.05-de,round(N/3)),linspace(1.05,maxE,round(N/3))))
        if self.x == None:
    ...
    def solveTheta(self):
        ...
        if self.shortappr:
            for e in enumerate(self.E):
                def thetafun(x): 
                    x = asarray(x)
                    Rpar = ones(len(x))*self.geometry.RL*self.geometry.RR/(self.geometry.RL+self.geometry.RR)
...
# The solver for fL:
    ...
    def solvefl(self):
        ...
        for e in enumerate(self.E):
            ...
            def dDl(x): return -2*imag(theta(array(x,ndmin=1))[:,1])*sin(imag(theta(array(x,ndmin=1)))[:,0])*cos(imag(theta(array(x,ndmin=1)))[:,0])
            ...
            def guess(x):
                z=array([[self.geometry.flR(e[1])*xx/self.geometry.L+self.geometry.flL(e[1])*(self.geometry.L-xx)/self.geometry.L, 0.1] for xx in x]).transpose()
                #dm = zeros((2,) + asarray(x).shape)
#                z = zeros((4,) + asarray(x).shape)
                dm = zeros((2,) + asarray(x).shape)
#                z[0] = 1
            ...
            solution=bvp.colnew.solve(boundaries, [2], fsub,gsub,tolerances=tol, is_linear=True,maximum_mesh_size=10000,is_complex=False, initial_guess=guess)
...
#######
    def solveft(self):
        ...
        for e in enumerate(self.E):
            ...
            def dDt(x): return 2*real(theta(array(x,ndmin=1))[:,1])*cosh(real(theta(array(x,ndmin=1))[:,0]))*sinh(real(theta(array(x,ndmin=1))[:,0]))
            def R(x): return -imag(sinh(theta(array(x,ndmin=1))[:,0]))
            def Ns(x): return real(cosh(theta(array(x,ndmin=1))[:,0]))
            def guess(x):
                z=array([[self.geometry.ftR(e[1])*xx/self.geometry.L+self.geometry.ftL(e[1])*(self.geometry.L-xx)/self.geometry.L, 0.1] for xx in x]).transpose()
                #dm = zeros((2,) + asarray(x).shape)
#                z = zeros((4,) + asarray(x).shape)
                dm = zeros((2,) + asarray(x).shape)
#                z[0] = 1
            ...
            solution=bvp.colnew.solve(boundaries, [2], fsub,gsub,tolerances=tol, is_linear=True,maximum_mesh_size=10000,is_complex=False)
...
#####
    ...
    def solveDelta(self):
        ...
        for x in enumerate(self.x):
            delta[x[0]] = trapz(self.fl[:,x[0],0]*real(sinh(self.theta[:,x[0],0])),self.E)
    ...
    def heatCurrent(self):
        ...
        if not self.solved['theta']: self.solveTheta()
        if self.approx_hc:
            ...
            jl = (self.geometry.flR(self.E) - self.geometry.flL(self.E)) * Ns0*NsL / \
                (self.geometry.RR*Ns0 + self.geometry.RL*NsL + Ns0*NsL*self.geometry.L/Dl)
            jt = Dt*(self.geometry.ftR(self.E)*NsL*Ns0*K - K*self.geometry.ftL(self.E)*Ns0 * (cosh(K*self.geometry.L)*NsL + Dt*K*self.geometry.RR*sinh(K*self.geometry.L))) / \
                (Dt*K*cosh(K*self.geometry.L)*(self.geometry.RR*Ns0+self.geometry.RL*NsL)+(Dt**2 * K**2 * self.geometry.RL*self.geometry.RR+Ns0*NsL)*sinh(K*self.geometry.L))
            # heat current = integral over E*jl + VL*jt
            integrand = self.E*jl-self.geometry.VL*jt
            return trapz(integrand, self.E)            
        
        
        if not self.solved['fl']: self.solvefl()
        if not self.solved['ft']: self.solveft()
