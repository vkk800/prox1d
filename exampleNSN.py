...
def generate_geometry(l, v): return geometry.geometry(
    TL=0.1, TR=0.1, RL=10000., RR=100., L=l, V=v, sys='NSN')
...
heatCurrentApprox = array([solver.solver(generate_geometry(
    L, v), bulkappr=True, approx_hc=True).heatCurrent() for v in V])
