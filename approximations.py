...
def polylog(n,x,N=40):
    sum = 0.
    for k in range(1,N): sum = sum + x**k / k**n
    return sum
# Fermi integral:
def g(n,x,N=40):
...
def INIS(sys, numerical=True):
    if not numerical:
        ...
        integrand = lambda e: abs(real(e/sqrt(e**2-1.)))*(1/(exp((e-sys.V)/sys.TL)+1)-1/(exp(e/sys.TR)+1))
...
def dQpe(sys):
    ...
    return -(tan(pi/8.)*exp(-sys.L*sqrt(2.)))*(2*sys.V**2-2*pi**2 / 3 * (sys.TL**2 - sys.TR**2))#-const2*(pi**2 * sys.TL**2 * sys.V**2 / 6 - sys.V**4 / 12)
...
def INIS_accurate(sys):
    ...
    N=1000
    de = 3./N
    E = concatenate((linspace(0.000001,0.95,round(N/3)),linspace(0.95+de,1.05-de,round(N/3)),linspace(1.05,maxE,round(N/3))))
    Ns=real((E+1j*gamma)/sqrt((E+1j*gamma)**2-1.))
    jt=zeros(len(E))
    Dt=0.5*(1+(E**2+gamma**2+1.)/((E**2+gamma**2)*abs(1-1./(E+1j*gamma)**2)))
    R=-2*1j*(E-1j*gamma-(E+1j*gamma)*sign(1-1./(E+1j*gamma)**2))/((E**2+gamma**2)*sqrt(1-1./(E+1j*gamma)**2))
    K=sqrt(R/Dt)
    ...
    jt=Dt*(K*sys.ftL(E)*Ns*(cosh(K*sys.L)*Ns+Dt*K*sys.RR*sinh(K*sys.L))-sys.ftR(E)*Ns**2 *K)/(Dt*K*cosh(K*sys.L)*(sys.RR*Ns+sys.RL*Ns)+(Dt**2 *K**2 *sys.RR*sys.RL+Ns**2)*sinh(K*sys.L))
...
def QNIS_accurate(sys):
    ...
    R=-1j*(E-1j*gamma-(E+1j*gamma)*sign(1-1./(E+1j*gamma)**2))/((E**2+gamma**2)*sqrt(1-1./(E+1j*gamma)**2))
    K=sqrt(R/Dt)
