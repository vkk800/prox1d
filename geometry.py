...
class geometry:
    ...
    def flL(self, E): return 0.5 * \
        (tanh((E-self.VL)/(2*self.TL))+tanh((E+self.VL)/(2*self.TL)))

    def flR(self, E): return 0.5 * \
        (tanh((E-self.VR)/(2*self.TR))+tanh((E+self.VR)/(2*self.TR)))
#    def flR(self, E): return tanh(E/(2*self.TR))

    def ftL(self, E): return 0.5 * \
        (tanh((E-self.VL)/(2*self.TL))-tanh((E+self.VL)/(2*self.TL)))

    def ftR(self, E): return 0.5 * \
        (tanh((E-self.VR)/(2*self.TR))-tanh((E+self.VR)/(2*self.TR)))
